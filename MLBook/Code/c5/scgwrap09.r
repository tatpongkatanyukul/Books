...
##
  ...
  wrap.term <- function(xk, term.data=NULL, term.info=NULL,
      doplot=FALSE, E.logs=NULL, nntype=nntype){
  ...
  #######################
   ...
   wsk.log <- scg(dE.w, mse.w, ws0, prob.info=prob.info, 
    ...
    MaxN=nEpochs, log=TRUE, nntype=nntype)

   net <- pack.w(wsk.log$x, D, M, K)
...
biclass.example <- function(){
  ...
  resSCGn <- nnTrain.scg(train.Xn,train.Y, nHiddens=20, nEpoch=500, nnType='biclass')

  resSCGnv <- nnTrain.scg(train.Xn,train.Y, nHiddens=20, nEpoch=500, nnType='biclass',
               val.X=val.Xn, val.T=val.Y)

  resSCGns <- nnTrain.scg(train.Xns,train.Y, nHiddens=20, nEpoch=500, nnType='biclass')
  ...
  yns <- nnOutput(resSCGns$net, test.Xn, nntype='biclass')  
  ...
  correct.ns <- sum( yns.class == test.Y )/length(test.Y) * 100

  cat('\ncorrect ratios:\n')
  print(c(correct.n, correct.nv, correct.ns))
##[1] 60.00000 66.66667 46.66667

}##end biclass.example  


multiclass.example <- function(zippath='./imageZIP'){
